...
'
'  Langkah:
'    1. Mapping data yang dikirim ke HTML (sementara hardcode: riders = {"SOC"}).
'    2. Pecah PDF → header.html, footer.html, bodyN.html, PageN.html  (Generator.Run).
'    3. Kirim data ke semua HTML → data.js (window.riplayData = {...}); PageN.html /
'       AllBody.html memuatnya lewat bind.js.
...
'       data dikirim langsung ke paginate(); blok yang tersembunyi tidak ikut).
...
Public Class RiplayGenerator
    ...
    ''' <summary>
    ''' Langkah 1 — data untuk template. Sementara hardcode; nanti diisi dari aplikasi.
    ''' Key dictionary = nama variabel yang ditulis di PDF:
    '''   riders → array string  → blok  <<if riders.contains('FEC')>> … <<endif>>        (halaman 1)
    '''   funds  → array objek   → baris tabel  <<funds.fundname>> | <<funds.funddesc>> | <<funds.fundrisk>>  (halaman 2)
    ...
    ''' </summary>
    Public Shared Function BuildData() As Dictionary(Of String, Object)
    ...
    Public Shared Async Function RunAsync(pdfPath As String, web As WebView2, log As Action(Of String)) As Task(Of String)
        ...
        log("Langkah 1: mapping data → " & json)
        ...
        ' ---- 3. kirim data ke semua HTML ----
        Dim dataJs = Path.Combine(outDir, "data.js")
        File.WriteAllText(dataJs,
            "// data.js — ditulis oleh Generate Riplay (langkah 3); dimuat semua PageN.html / AllBody.html lewat bind.js" & vbLf &
            "window.riplayData = " & json & ";" & vbLf, Utf8NoBom)
        log("Langkah 3: data.js ← " & json & "  (dipakai semua PageN.html / AllBody.html)")

        ' ---- 4. kondisi & baris berulang di halaman ----
        Dim fundCount = DirectCast(data("funds"), Array).Length
        log("Langkah 4: bind.js menerapkan data saat halaman dibuka — riders = " & JsonSerializer.Serialize(data("riders")) &
            " → blok <<if riders.contains(…)>>; funds = " & fundCount & " item → baris tabel <<funds.…>>")
        ...
        log("Langkah 6: header.html + footer.html + AllBody.html → AllPages.html")
